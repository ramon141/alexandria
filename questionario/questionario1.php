<?php
	...
	while ($fetch = mysqli_fetch_array($query)){
		if($loop == $perguntaAtual){
			...
			$queryvNotas = mysqli_query($connection, "SELECT * FROM vnotas where modulo_idModulo = '1' and usuario_idUsuario = '$idUsuario' and idPergunta = '$fetch[0]'");
			$temm = mysqli_num_rows($queryvNotas);
			if($temm >= 1){
				$_SESSION['respostaUltima'] = -1;
				$_SESSION['perguntaAtual'] = $_SESSION['perguntaAtual'] + 1;
				echo "Essa pergunta já foi respondida<br>";
				echo "<a href=\"questionario1.php?r=1\">Próxima</a><br>";
				exit();
